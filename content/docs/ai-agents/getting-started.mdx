...
The wizard installs the skill for you. To check installed skills and update any
outdated copies later without rerunning the wizard, use:

```sh
but skill check --update
```

See [`but skill`](/commands/but-skill) for other install and update options.
