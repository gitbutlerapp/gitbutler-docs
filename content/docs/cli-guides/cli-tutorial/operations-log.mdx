...
You can easily restore to any point in the operations history by running the `but restore` command. If it will modify your working directory (maybe it's just changes to the commit history or staging stuff), then it will prompt you if you want to continue. If you don't want that, you can pass `-f` or `--force` to make it do it anyhow.

```cli [operations-log-but-restore-1, 176px]
