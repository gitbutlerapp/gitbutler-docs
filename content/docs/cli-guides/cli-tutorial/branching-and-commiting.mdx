...
description: Create branches, commit changes, and stack dependent work with the GitButler CLI.
---
...
We can see our previous branch and the commit we made, our new empty branch and a couple of modified files. Now we can commit the unassigned changes to that branch with `but commit -b liked-tweets -m "liked tweets changes"`.

```cli [branching-and-commiting-but-commit-2, 88px]
but commit -b liked-tweets -m "liked tweets changes"
```
...
The other way you can create new branches is to stack them. One branch depends
on another, but each branch can still be reviewed independently.
...
but commit -b liked-tweets-stacked -m "liked tweets changes"
```
...
Once you have connected a forge account with `but config forge auth`, publish
the whole stack from its top branch:

```bash
but pr new liked-tweets-stacked -t
```

GitButler pushes both branches and opens a review for each one, with
`liked-tweets-stacked` targeting `user-bookmarks`. See
[Create stacked branches and pull requests](/features/branch-management/stacked-branches)
for how GitButler keeps the stack in sync.
