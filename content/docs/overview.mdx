...
This ability to do multiple dependent or independent branches at the same time is something that is not possible with vanilla Git.
...
<Callout type="warn">
  When using GitButler, we will setup a special branch to handle the multiple branches in a way that
...
<Callout type="info">
  ...
  [integration branch](/features/branch-management/integration-branch) docs. We are working on making
