...
When `<BRANCH>` is the top of a stack, `but pr new <BRANCH>` pushes the whole
stack and creates missing reviews from the bottom upward. GitButler sets each
review's base to the nearest branch below it that also has a review.

Add `-t` to skip the title and description prompts. `-m` and `-F` apply only to
`<BRANCH>`; branches below it use their default titles and descriptions.

After creating the reviews, GitButler updates either GitHub's native stack
membership or the stack information in the review descriptions. See
