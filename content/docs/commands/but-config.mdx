...
View or configure native GitHub stacked pull requests for the current
repository.
...
**Usage:** `but config forge github-stacks [STATUS]`

**Arguments:**

* `<STATUS>` — `auto` (default), `enable`, or `disable`. Stored in the
  repository's Git config and shared with GitButler Desktop.
